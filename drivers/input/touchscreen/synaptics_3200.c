...
#define DT2W_DELTA 115
#define L2W_TIMEOUT 25
...
static void sweep2wake_vert_func(int x, int y)
{
	...
	if (firsty > 1139)
		reset_sv2w();
	//sweep up
	if ((x > 50 && x < 750) && (gestures_switch || (s2w_switch & SWEEP_UP))) {
		if (firsty > 750) {
			prevy = firsty;
			nexty = prevy - 85;
		...
		//sweep down
		} else if ((firsty <= 750) && (gestures_switch || (s2w_switch & SWEEP_DOWN))) {
			prevy = firsty;
			nexty = prevy + 85;
			if (barriery[0] == true || (y > prevy && y < nexty)) {
				prevy = nexty;
...
static void sweep2wake_horiz_func(int x, int y)
{
	...
	//sweep right
	if (firstx < 405 && (gestures_switch || (s2w_switch & SWEEP_RIGHT))) {
		prevx = firstx;
		nextx = prevx + 90;
	...
	//sweep left
	} else if (firstx >= 405 && y < 1395 && (gestures_switch || (s2w_switch & SWEEP_LEFT))) {
		prevx = firstx;
		nextx = prevx - 90;
		if ((barrierx[0] == true) ||(x < prevx && x > nextx && y < 1395)) {
			prevx = nextx;
			nextx -= 100;
			barrierx[0] = true;
			if ((barrierx[1] == true) || (x < prevx && x > nextx  && y < 1395)) {
				prevx = nextx;
				barrierx[1] = true;
				if (x < prevx  && y < 1395) {
					if (x < (nextx - 90)) {
...
static int report_htc_logo_area(int x, int y) {
	...
	if (last_touch_position_x > 310 && last_touch_position_x < 575) {
		if (last_touch_position_y > 1417 || (scr_suspended == true && last_touch_position_y > 1375)) {
...
{
        if ((x > 0 && x < 75) || x > 735 || y > 1440 || (!gestures_switch && dt2w_switch == 1 && (y > 0 && y < 1050))) {
                reset_dt2w();
                return;
        }
